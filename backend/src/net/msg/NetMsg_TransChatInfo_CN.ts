// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: NetMsg_TransChatInfo_CN

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  TransChatInfoRpt,
} from 'mc-local-share';

/**
 * NetMsg_TransChatInfo_CN
 * REQ = TransChatInfoRpt
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=15029、recId=-1
 */
export class NetMsg_TransChatInfo_CN implements IHandle<TransChatInfoRpt, {}> {
  /** 请求消息号：TRANS_CHAT_INFO_RPT (15029) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.TRANS_CHAT_INFO_RPT;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: TransChatInfoRpt): {} {
    throw new Error('Handle not implemented: NetMsg_TransChatInfo_CN');
  }
}
