// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: NetMsg_GetLoginActivityRewardReq

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  GetLoginActivityRewardReq,
} from 'mc-local-share';

/**
 * NetMsg_GetLoginActivityRewardReq
 * REQ = GetLoginActivityRewardReq
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=10342、recId=-1
 */
export class NetMsg_GetLoginActivityRewardReq implements IHandle<GetLoginActivityRewardReq, {}> {
  /** 请求消息号：GET_LOGIN_ACTIVITY_REWARD_REQ (10342) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.GET_LOGIN_ACTIVITY_REWARD_REQ;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: GetLoginActivityRewardReq): {} {
    throw new Error('Handle not implemented: NetMsg_GetLoginActivityRewardReq');
  }
}
