// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: NetMsg_GetShareDecks_CN

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  GetSharedDecksReq,
} from 'mc-local-share';

/**
 * NetMsg_GetShareDecks_CN
 * REQ = GetSharedDecksReq
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=10360、recId=-1
 */
export class NetMsg_GetShareDecks_CN implements IHandle<GetSharedDecksReq, {}> {
  /** 请求消息号：GET_SHAREDDECKS_REQ (10360) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.GET_SHAREDDECKS_REQ;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: GetSharedDecksReq): {} {
    throw new Error('Handle not implemented: NetMsg_GetShareDecks_CN');
  }
}
