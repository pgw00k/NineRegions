// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: NetMsg_InfiSelectStage

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  InfiSelectStageRequest,
} from 'mc-local-share';

/**
 * NetMsg_InfiSelectStage
 * REQ = InfiSelectStageRequest
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=10088、recId=-1
 */
export class NetMsg_InfiSelectStage_CN implements IHandle<InfiSelectStageRequest, {}> {
  /** 请求消息号：INFI_SELECT_STAGE_REQ (10088) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.INFI_SELECT_STAGE_REQ;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: InfiSelectStageRequest): {} {
    throw new Error('Handle not implemented: NetMsg_InfiSelectStage');
  }
}
