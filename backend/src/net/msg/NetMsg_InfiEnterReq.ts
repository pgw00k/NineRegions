// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: Infi_EnterReq

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  InfiEnterRequest,
} from 'mc-local-share';

/**
 * Infi_EnterReq
 * REQ = InfiEnterRequest
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=10080、recId=-1
 */
export class NetMsg_InfiEnterReq implements IHandle<InfiEnterRequest, {}> {
  /** 请求消息号：INFI_ENTER_REQ (10080) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.INFI_ENTER_REQ;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: InfiEnterRequest): {} {
    throw new Error('Handle not implemented: Infi_EnterReq');
  }
}
