// 由 mc-local-share generate_res 自动生成，请勿手改。
// tagName: Infi_GetStaticDataReq

import { IHandle } from '../IHandle';
import {
  MESSAGE_ID,
  InfiGetStaticDataReq,
} from 'mc-local-share';

/**
 * Infi_GetStaticDataReq
 * REQ = InfiGetStaticDataReq
 * RES = {}（recvProto 缺失，回退）
 * 说明：RES 使用 {} 作为占位。（未声明 recvProto）
 * 注册：reqId=10116、recId=-1
 */
export class NetMsg_InfiGetStaticDataReq implements IHandle<InfiGetStaticDataReq, {}> {
  /** 请求消息号：INFI_GET_STATICDATA_REQ (10116) */
  readonly reqId: MESSAGE_ID = MESSAGE_ID.INFI_GET_STATICDATA_REQ;
  /** 响应消息号：-1 */
  readonly recId: number = -1;

  Handle(req: InfiGetStaticDataReq): {} {
    throw new Error('Handle not implemented: Infi_GetStaticDataReq');
  }
}
